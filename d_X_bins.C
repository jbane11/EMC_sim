...
void d_X_bins(int run1=0, int run2=0,int quite =1,int quick =1) {
		time_t start = time(0) ;
...
////////////Find the values for selected bins of x.
	...
	char xfile[100];
	int xf = sprintf(xfile, "%s/dXbins_%d_%d.txt",data_dir,run2,run1);
	ofstream xbins;
	xbins.open(xfile);
...
/////////////////////////
	...
	if(quite==0){cout<<setprecision(3)<<setw(5) <<ratio[k]<<" ";}
