...
#include "/u/apps/root/5.34.36/root/include/snprintf.h"
#include "/u/apps/root/5.34.36/root/include/TTree.h"

#include "/u/apps/root/5.34.36/root/include/TSystem.h"
#include "/u/apps/root/5.34.36/root/include/TString.h"
#include "/u/apps/root/5.34.36/root/include/TCanvas.h"
#include "/u/apps/root/5.34.36/root/include/TCut.h"
#include "/u/apps/root/5.34.36/root/include/TFile.h"
#include "/u/apps/root/5.34.36/root/include/TH1F.h"
#include "/u/apps/root/5.34.36/root/include/TChain.h"
#include "/u/apps/root/5.34.36/root/include/TF1.h"
#include "/u/apps/root/5.34.36/root/include/TNtuple.h"
#include "/u/apps/root/5.34.36/root/include/TRandom.h"
#include "/u/apps/root/5.34.36/root/include/TRandom3.h"
...
using namespace std;
const 	int number_of_electrons =100; 
//#include "nr3.h"
//#include "ran.h"
...
//	int run	= atoi(runr);
//	int run1= atoi(runr1);
void sim_one_mt(int run, int run1){
	int no_O=1;	
...
cout << "Please input the run number, you would like to call this run." <<"\n";;
	...
	int n = sprintf(output_file,"%s/IS_%d.root",root_dir,run1);
...
//Input for momentun distribution
	...
	ifstream lists;
	lists.open(chart);
	if (!lists.good()) {cout << "error invalid av18 dist" <<endl;return ;}
	cout << "\n" << "\n";
...
// Constants
	...
	int print = 0; //If you want to check the 4 vectors -> 10;
	...
	double deltaE_rest;
	double Qsquared; 	double xb; double Xb_rest; double Xb_lab;
	double invar_mass ;	double invar_mass_RF;
	double F_two_lab ; 	double F_two_mod_lab;
	double diff_cross_lab;	double diffcross_mod_lab;
	double theta; 	double F_two; 	double QQ ; double x ;
	double F_two_mod ;
	double eErf;	 // Electron energy after scattering in rest frame!
	double ISE ; 	double nuw; 	double Eloss;
	double efinal_lab;
	cout << "\n" << "\n";
/////////////////////////////////////////////	
...
TTree *tree = new TTree("tree",Form("Tree for run %d", run));
		...
		tree->Branch("Prot_momentum",&Prot_momentum,"Prot_momentum/D");
	 	...
	 	tree->Branch("qsquared",&Qsquared,"qsquared/D");
	 	tree->Branch("Xb_rest",&Xb_rest,"Xb_rest/D");
	 	//tree->Branch("Xa_rest",&Xa_rest,"Xa_rest/F");
	 	tree->Branch("Sigma_rest",&diff_cross_lab,"Sigma_rest/D");
	 	tree->Branch("F_two",&F_two,"F_two/D");
	 	tree->Branch("theta_lab",&theta,"theta_lab/D");
	 	tree->Branch("Beam",&elec_Beam_momentum,"Beam/D");
	 	tree->Branch("efinal_lab",&efinal_lab,"efinal_lab/D");
	 	tree->Branch("Xb_lab",&Xb_lab,"Xb_lab/D");
		tree->Branch("nuw",&nuw,"nuw/I");	 	
		tree->Branch("invar_mass",&invar_mass,"invar_mass/D");
...
#endif
	cout <<"Max threads " <<maxthr<<endl;
	cout <<"Number of threads " << numthr <<endl;
#ifndef __CINT__
#pragma omp parallel  
#endif
{
...
/*
if(i/(number_of_electrons/2) == i/(number_of_electrons*0.5)){
  	 		 time_t rawtime1;
	  		struct tm * timeinfo1;
	  		time (&rawtime1);
...
int which_thr;
#ifndef __CINT__
which_thr =omp_get_thread_num();
 #endif
if(i/(number_of_electrons/10) == i/(number_of_electrons*0.1)){cout << i <<"  "<<which_thr <<" "<<"\n";}
//tree->Fill();
scatt++;
